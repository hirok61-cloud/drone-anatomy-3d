...
document.getElementById('xHeat').click(); d.step(6.5, 30); A(X.heat > 0.95 && document.querySelectorAll('#heatBars .hb').length === 9, 'sensors: heat', X.heat);
...
const r = window.__toolboxCalc ? __toolboxCalc(tb) : null; A(!r || (r.TW > 2 && r.TW < 4 && r.tmin > 8 && r.tmin < 30 && r.Imax > 60), 'toolbox: plausible', r && r.TW, r && r.tmin);
...
A(X.bladeDiscs.every(m => !m.visible) && (!X.donut || !X.donut.visible) && esc.meshes[0].material === esc.meshes[0].userData.origMat && frame.meshes[0].material === frame.meshes[0].userData.origMat, 'leave: visuals cleared');
d.noRender = false;
log.length ? log : '⑪⑫⑬ OK';
