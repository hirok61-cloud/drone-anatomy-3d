...
A(document.querySelectorAll('#whatifCards .ky').length === WHATIF.length && document.querySelectorAll('#mishapCards .ky').length === document.querySelectorAll('#mishapCards button').length, 'A3 card chips');
document.getElementById('kyBtn').click(); d.step(0.2, 30); A(document.getElementById('noteCard').dataset.kind === 'kyosoku' && document.querySelectorAll('#noteCard table.ky-tbl tr').length === R.KY_TAB.length + 1 && /第5版/.test(document.getElementById('noteTitle').textContent), 'A3 table'); document.getElementById('noteClose').click();
// A4 もしも → 制度では / A6 報告するなら
document.querySelector('#whatifCards button[data-w=person]').click(); d.step(1.0, 30); document.getElementById('thNext').click(); let g = 0; while (d.__whatif.phase < 3 && g++ < 40) d.step(0.5, 30);
...
// A5 立入管理措置の場面
document.querySelector('#whatifCards button[data-w=tachiiri]').click(); d.step(1.2, 30); const W = d.__whatif;
...
// オフにすると消える
R.setReg(false); d.step(0.2, 30); A(document.querySelectorAll('#whatifCards .ky').length === 0, 'off hides chips'); R.setReg(true);
d.noRender = false;
log.length ? log : 'A OK';
