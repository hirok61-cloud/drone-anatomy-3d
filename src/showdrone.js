...
const SD = {
  root: null, built: false, leds: [], discs: [], props: [], mat: null,
...
function buildShowDrone(M) {
  ...
  const Y_PROP = 0.0355;
  const white = new THREE.MeshStandardMaterial({ color: 0xeef0f3, roughness: 0.52, metalness: 0.0 });
  const cage = new THREE.MeshStandardMaterial({ color: 0xe4e7ec, roughness: 0.86, metalness: 0.0 });
  ...
  // --- モーターとプロペラ ---
  {
    ...
    for (let k = 0; k < 4; k++) {
      const cx = (k === 0 || k === 3 ? 1 : -1) * MO, cz = (k < 2 ? 1 : -1) * MO;
      const ccw = (k === 0 || k === 2);
      const dm = (ccw ? M.propDiscCCW : M.propDiscCW).clone(); dm.opacity = 0.34;   /* 飛んでいる状態。ぼかし円盤で回転を表す */
  ...
  // --- 灯り。これが本体で、機体はそれを運ぶ台に見えるようにする ---
  {
    ...
    g.add(put(mesh(cylGeo(0.0200, 0.0188, 0.0075, 24), new THREE.MeshStandardMaterial({ color: 0xb9bec7, roughness: 0.55, metalness: 0.35 })), 0, 0.0225, 0));   /* 放熱のえり */
