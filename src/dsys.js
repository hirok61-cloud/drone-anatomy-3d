...
function dsysFence() {   // ⑥囲い: 内側（越えたら帰る）と外側（越えたら止める）
  ...
  const y0 = -DSHOW_H * (1 - k) * (1 - gz) / k, y1 = DSHOW_H + DSHOW_R * 0.94;
  for (const [r, lm, nv] of [[DSHOW_R * 1.10, dsys.mats.line, 8], [DSHOW_R * 1.55, dsys.mats.warnLine, 16]]) {
    ...
    lab.position.set(0, y1 - DSHOW_R * (inner ? 0.18 : 0.50), r * 0.62); dsys.air.add(lab);   /* 手前側に寄せる。上に出すと画面の外へ出る */
  }
  // 外側のさらに外。落ちた機体が届く範囲で、人を入れない区画。
  // 本文の締めがここなのに地面に何も無いと、いちばん効く一文だけ絵で確かめられない
  // dsys.world は倍率 k がかかるので、隊列の座標をそのまま使える
  { const r2 = DSHOW_R * 1.55, r3 = r2 + DSHOW_H * 0.30;   /* 外側の枠の外へ、高さに応じて広がる */
    ...
    const lab = dsysLabel('落ちた機体が届く範囲 ＝ 人を入れない区画', DSHOW_R * 1.36);
    lab.position.set(0, 0.42, r2 * 0.74); lab.material.depthTest = false; dsys.world.add(lab);
  }
}
