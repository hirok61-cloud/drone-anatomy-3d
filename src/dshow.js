...
const DSHOW_N = () => (S.qLevel <= 1 ? 600 : S.qLevel === 2 ? 1200 : 2000);   // 実際のショーは数百〜数千機
const DSHOW_H = 3.10;      // 隊列の中心の高さ(m)
...
function dshowShape(id, n, out) {
  ...
  if (id === 'grid') {
    ...
    const t = id.slice(5);
    dshowFromCanvas((c, W, H) => { c.font = `900 ${Math.round(H * 0.78)}px "Noto Sans JP", ${'sans-serif'}`; c.textAlign = 'center'; c.textBaseline = 'middle'; c.fillText(t, W / 2, H * 0.54); },
...
// 走査順に並べると遷移のとき点が帯になって動くので、候補をシャッフルしてから配る。
const _dsCv = { el: null, ctx: null };
function dshowFromCanvas(draw, n, out, opts) {
  const o = opts || {}, W = o.w || 220, H = o.h || 140, S1 = o.scale || (DSHOW_R * 2 / W);
  ...
  draw(c, W, H);
  const px = c.getImageData(0, 0, W, H).data, cand = [];
  for (let y = 0; y < H; y++) for (let x = 0; x < W; x++) if (px[(y * W + x) * 4] > 120) cand.push(x + y * W);
  ...
  for (let i = 0; i < n; i++) {
    const k = cand[i % cand.length], x = k % W, y = (k / W) | 0;
    const jx = i >= cand.length ? (Math.sin(i * 7.13) * 0.5) : 0, jy = i >= cand.length ? (Math.cos(i * 5.71) * 0.5) : 0;
    out[i * 3] = (x + 0.5 + jx - W / 2) * S1;
    out[i * 3 + 1] = DSHOW_H - (y + 0.5 + jy - H / 2) * S1;
    out[i * 3 + 2] = 0;
  }
}
...
function dshowBuild() {
  ...
  const mat = new THREE.PointsMaterial({ size: 0.135, map: canvasTex(glowCanvas(true)), vertexColors: true, transparent: true, depthWrite: false, blending: THREE.AdditiveBlending, sizeAttenuation: true, toneMapped: false });
...
function dshowOn(on) {
  ...
  if (on) {
    ...
    dshowSky();
    { const tall = dshowTall();
      const dir = new THREE.Vector3(0.32, 0.40, 1).normalize(), aim = new THREE.Vector3(0, tall ? 2.25 : 2.55, 0);   /* 見上げるほど空が広い */   /* 観客の位置から見上げる。隊列と地上の機体を1枚に入れる */
      flyTo(aim.clone().addScaledVector(dir, (tall ? 13.8 : 15.6) * viewScale()), aim, 1100, false); }
    renderDshow();
