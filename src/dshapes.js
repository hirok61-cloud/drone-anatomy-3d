  ...
  const gen = (g) => {
    ...
    for (const c of chains) {
      ...
      for (let q = 0; q < cnt; q++) { const s = (q + 0.5) / cnt * c.len; while (j < c.m - 1 && c.acc[j + 1] < s) j++;
        ...
        const j2 = (j + 1) % c.m, t = (s - c.acc[j]) / Math.max(1e-6, c.acc[j + 1] - c.acc[j]), x = c.xs[j] + (c.xs[j2] - c.xs[j]) * t, y = c.ys[j] + (c.ys[j2] - c.ys[j]) * t;
...
function dshapeMake(src, n, env) {
  ...
  if (!env.still) {
    ...
    for (const a of list) { const o = Object.assign({}, a);
...
function _dsSpineBind(a, sh, pts) {
  ...
  for (let i = 0; i < n; i++) { if (!a.m[sh.tag[i]]) continue; const x = sh.bx[i], y = sh.by[i]; let bj = 0, bd = 1e9;
    for (let j = 0; j <= M; j++) { const d = (cv.x[j] - x) * (cv.x[j] - x) + (cv.y[j] - y) * (cv.y[j] - y); if (d < bd) { bd = d; bj = j; } }
